...
interface GaussianSplatDirectURLProps {
  assetUrl?: string;
}

export function GaussianSplatDirectURL({ assetUrl }: GaussianSplatDirectURLProps) {
  console.log('🎬 SparkJS Direct URL component rendered with assetUrl:', assetUrl);
  ...
  useEffect(() => {
    ...
    console.log('🎬 Loading splat file with direct URL:', assetUrl);
    setIsLoading(true);
    setError(null);
    ...
    // Try using the direct URL first
    try {
      console.log('🎬 Creating SplatMesh with direct URL...');
      ...
      console.log('✅ SplatMesh created and added to scene with direct URL:', splatMesh);
      setIsLoading(false);
    } catch (err) {
      console.error('❌ Error creating SplatMesh with direct URL:', err);
      setError(`Failed to create SplatMesh with direct URL: ${err}`);
      setIsLoading(false);
    }
  }, [assetUrl]);
